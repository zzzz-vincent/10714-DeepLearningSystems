...
// BLOCK_SIZE are defined by heuristic.
...
CudaDims CudaTwoDim(size_t size_x, size_t size_y) {
   ...
   */
  CudaDims dim;
  size_t num_blocks_x = (size_x + BLOCK_SIZE - 1) / BLOCK_SIZE;
  size_t num_blocks_y = (size_y + BLOCK_SIZE - 1) / BLOCK_SIZE;
...
__device__ size_t getIndex(const CudaVec& shape, const CudaVec& strides, size_t idx) {
  int32_t out_index = 0;
  int32_t cur_ptr = shape.size - 1;
  while (idx != 0) {
    size_t val = idx % shape.data[cur_ptr];
    idx = idx / shape.data[cur_ptr];
    out_index += strides.data[cur_ptr] * val;
    cur_ptr--;
  }
  return out_index;
}
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size) {
    out[gid] = a[offset + getIndex(shape, strides, gid)];
  }
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
                              CudaVec strides, size_t offset) {
  /**
   * The CUDA kernel for the elementwise set opeation. This should effectively map a single entry in the 
   * non-compact input a, to the corresponding item (at location gid) in the compact array out.
   * 
   * Args:
   *   a: CUDA pointer to a array
  ...
  if (gid < size) {
    out[offset + getIndex(shape, strides, gid)] = a[gid];
  }
}
...
__global__ void ScalarSetitemKernel(scalar_t val, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size) {
    out[offset + getIndex(shape, strides, gid)] = val;
  }
}
...
__device__ void EwiseOperationKernel(const scalar_t* a, scalar_t* out, size_t size, scalar_t(* op)(scalar_t)) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    out[gid] = op(a[gid]);
  }
...
__device__ void EwiseOperationKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, size_t size, scalar_t(* op)(scalar_t, scalar_t)) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
...
__device__ void ScalarOperationKernel(const scalar_t* a, scalar_t val, scalar_t* out, size_t size, scalar_t(* op)(scalar_t, scalar_t)) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
...
__global__ void EwiseAddKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, size_t size) {
  EwiseOperationKernel(a, b, out, size, &ADD);
}
...
__global__ void ScalarAddKernel(const scalar_t* a, scalar_t val, scalar_t* out, size_t size) {
  ScalarOperationKernel(a, val, out, size, &ADD);
}
...
__global__ void EwiseMulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, size_t size) {
  EwiseOperationKernel(a, b, out, size, &MUL);
}
...
__global__ void ScalarMulKernel(const scalar_t* a, scalar_t val, scalar_t* out, size_t size) {
  ScalarOperationKernel(a, val, out, size, &MUL);
}
...
__global__ void EwiseDivKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, size_t size) {
  EwiseOperationKernel(a, b, out, size, &DIV);
}
...
__global__ void ScalarDivKernel(const scalar_t* a, scalar_t val, scalar_t* out, size_t size) {
  ScalarOperationKernel(a, val, out, size, &DIV);
}
...
__global__ void ScalarPowerKernel(const scalar_t* a, scalar_t val, scalar_t* out, size_t size) {
  ScalarOperationKernel(a, val, out, size, &POWER);
}
...
__global__ void EwiseMaximumKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, size_t size) {
  EwiseOperationKernel(a, b, out, size, &MAX);
}
...
__global__ void ScalarMaximumKernel(const scalar_t* a, scalar_t val, scalar_t* out, size_t size) {
  ScalarOperationKernel(a, val, out, size, &MAX);
}
...
__global__ void EwiseEqKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, size_t size) {
  EwiseOperationKernel(a, b, out, size, &EQ);
}
...
__global__ void ScalarEqKernel(const scalar_t* a, scalar_t val, scalar_t* out, size_t size) {
  ScalarOperationKernel(a, val, out, size, &EQ);
}
...
__global__ void EwiseGeKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, size_t size) {
  EwiseOperationKernel(a, b, out, size, &GE);
}
...
__global__ void ScalarGeKernel(const scalar_t* a, scalar_t val, scalar_t* out, size_t size) {
  ScalarOperationKernel(a, val, out, size, &GE);
}
...
__global__ void EwiseLogKernel(const scalar_t* a, scalar_t* out, size_t size) {
  EwiseOperationKernel(a, out, size, &LOG);
}
...
__global__ void EwiseExpKernel(const scalar_t* a, scalar_t* out, size_t size) {
  EwiseOperationKernel(a, out, size, &EXP);
}
...
__global__ void EwiseTanhKernel(const scalar_t* a, scalar_t* out, size_t size) {
  EwiseOperationKernel(a, out, size, &TANH);
}
...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, uint32_t m, uint32_t n, uint32_t p) {
  __shared__ scalar_t shared_a[BLOCK_SIZE][BLOCK_SIZE];
  __shared__ scalar_t shared_b[BLOCK_SIZE][BLOCK_SIZE];
  ...
  for (size_t n_i = 0; n_i < n; n_i += BLOCK_SIZE) {
    // Cooperative Fetching
    __syncthreads();
    if (tid_x < BLOCK_SIZE && tid_y < BLOCK_SIZE) {
      if (tid_y + n_i < n && tid_x + block_x < m) {
        shared_a[tid_x][tid_y] = a[n * (block_x + tid_x) + tid_y + n_i];
      } else {
        // if shared_a[tid_x][tid_y] has out of Matrix A, sets it to 0
        shared_a[tid_x][tid_y] = 0;
      }
      if (tid_y + block_y < p && tid_x  + n_i < n) {
        shared_b[tid_x][tid_y] = b[p * (tid_x + n_i) + tid_y + block_y];
      } else {
        // if shared_a[tid_x][tid_y] has out of Matrix A, sets it to 0
        shared_b[tid_x][tid_y] = 0;
      }
    }
    __syncthreads();

    // Computes block-wise dot product and update it to out
    for (size_t b_i = 0; b_i < BLOCK_SIZE; b_i++) {
        if (block_x + tid_x < m && block_y + tid_y < p) {
          out[(block_x + tid_x) * p + block_y + tid_y] += (shared_a[tid_x][b_i] * shared_b[b_i][tid_y]);
        }
    }
  }
}
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  cudaMemset(out->ptr, 0, out->size * sizeof(scalar_t));
...
__device__ void ReduceOperationKernel(const scalar_t* a, scalar_t* out, size_t step, size_t size, scalar_t(* op)(scalar_t, scalar_t)) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    for (size_t i = 0; i < step; i++) {
      size_t idx = step * gid + i;
      if (i == 0) {
        out[gid] = a[idx];
    } else {
        out[gid] = op(out[gid], a[idx]);
      }
    }
  }
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t size) {
  ReduceOperationKernel(a, out, reduce_size, size, &MAX);
}
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t size) {
  ReduceOperationKernel(a, out, reduce_size, size, &ADD);
}
