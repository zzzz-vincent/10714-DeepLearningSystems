...
def epoch_general_ptb(data, model, seq_len=40, loss_fn=nn.SoftmaxLoss(), opt=None,
    ...
    for i in range(0, nbatch - 1, seq_len):
      ...
      if isinstance(h_n, tuple):
        h_n = (ndl.Tensor(h_n[0].detach().numpy(), device=device, dtype=dtype), ndl.Tensor(
                h_n[1].detach().numpy(), device=device, dtype=dtype))
      else:
        h_n = ndl.Tensor(h_n.detach().numpy(), device=device, dtype=dtype)
      loss = loss_fn(out, y)
      total_loss += loss.detach().numpy() * num
