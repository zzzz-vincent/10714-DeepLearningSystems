...
class NDArray:
    ...
    def squeeze(self, axis):
        ...
        if axis == None:
          return self
        if (axis > self.ndim) or (axis < 0) or (self.shape[axis] != 1):
    ...
    def reshape(self, new_shape):
        ...
        if old_items != new_items:
          raise ValueError("cannot reshape array of size {} into shape {}".format(old_items, new_shape))
    ...
    def transpose(self, new_axes=None):
        ...
        if not new_axes:
          return self.permute(tuple([i for i in range(self.ndim - 1, -1, -1)]))
    ...
    def __getitem__(self, idxs):
        ...
        return self.make(tuple(new_shape), strides=tuple(new_strides), device=self.device, handle=self._handle, offset=new_offset)
    ...
    def reduce_view_out(self, axis, keepdims=False):
        ...
        if axis is None:
            ...
            reduced = [s for i, s in enumerate(self.shape) if i in axis]
            out_shape.append(prod(reduced))
            view = self.permute(
                tuple([a for a in range(self.ndim) if a not in axis]) + tuple(axis)
            ).compact().reshape(out_shape)
            out = NDArray.make(
    ...
    def mean(self, axis=None, keepdims=False):
        sum_val = self.sum(axis, keepdims)
        size = 1
        if axis is None:
          size = self.size
          ...
          size = prod([self.shape[i] for i in axis])
        return sum_val / size
    ...
    def flip(self, axes):
        ...
        if axes is None:
          axes = [i for i in range(self.ndim)]
        for axis in axes:
          new_offset += self._strides[axis] * (self._shape[axis] - 1)
          new_strides[axis] *= -1
        out = self.make(list(self.shape), strides=new_strides, device=self.device, handle=self._handle, offset=new_offset)
        return out.compact()
    ...
    def pad(self, axes):
        ...
        for old_shape, (pre_pad, pos_pad) in zip(self.shape, axes):
          assert pre_pad >= 0 and pos_pad >= 0
          new_shape.append(old_shape + pre_pad + pos_pad)
          slices.append(slice(pre_pad, pre_pad+old_shape, 1))
