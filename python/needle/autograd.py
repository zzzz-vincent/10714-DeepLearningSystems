...
class Op:
    ...
    def gradient_as_tuple(self, out_grad: "Value", node: "Value") -> Tuple["Value"]:
        ...
        # else:
        #   print("out_grad", out_grad[0].shape)
        # for i in node.inputs:
        #   if isinstance(i, Tensor):
        #     print("input", i.shape)
        #   else:
        #     print("input", i[0].shape, "x", len(i))
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
      ...
      if node.is_leaf():
        continue
      for prev_grad, prev_input in zip(node.op.gradient_as_tuple(node.grad, node), node.inputs):
        if prev_input not in node_to_output_grads_list:
          node_to_output_grads_list[prev_input] = []
        node_to_output_grads_list[prev_input].append(prev_grad)
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    for i in range(len(inputs)):
      topo_sort_dfs(inputs[i], visited, topo_order)
    visited.add(node)
